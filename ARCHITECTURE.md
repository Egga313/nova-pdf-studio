...
- [x] Phase 7 — offline OCR (tesseract.js in the main process, language packs in `resources/tessdata`, ara/fra/eng), OCR dialog in the viewer (page/document, progress, text, make searchable → text layer + `document_pages`, searchable-PDF export with an invisible Noto Naskh layer), PDF → smart invoice tab (auto field/item detection in ar/fr/en, draw zones + item columns, calculation checks, extraction templates in `invoice_templates` kind `extraction` with automatic recognition by keywords, customer matching, invoice prefill), batch import of many PDFs as draft invoices
