...
- [x] Phase 7 — offline OCR (tesseract.js in the main process, language packs in `resources/tessdata`, ara/fra/eng), OCR dialog in the viewer (page/document, progress, text, make searchable → text layer + `document_pages.text_content/layout` so search works again after reopening, searchable-PDF export with an invisible Noto Naskh layer; tesseract.js runs through `resources/ocr/worker.cjs` because it otherwise treats Electron as a browser and fetches over the network), PDF → smart invoice tab (auto field/item detection in ar/fr/en, draw zones + item columns, calculation checks, extraction templates in `invoice_templates` kind `extraction` with automatic recognition by keywords, customer matching, invoice prefill), batch import of many PDFs as draft invoices
- [x] Phase 8 — audit log tab (filters, CSV export, jump to object), global search in the command palette (customers/invoices/products), internal notes + attachments (copied into the data folder) + custom fields (definitions in Settings → Custom fields, values on invoices and customers), autosave recovery drafts (`drafts` table, banner in the invoices list, restore after a crash), removal of the non-functional "protect" tool, RTL polish; 88+ unit tests across money, numbering, calc, amount-in-words, PDF tools/edits, templates, formulas, sheets, xlsx, extraction
